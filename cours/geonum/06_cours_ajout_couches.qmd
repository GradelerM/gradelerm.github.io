...
La partie `~/Documents/Cours/GeoNum/Webmapping/geonum_docker$` et le **chemin absolu** vers mon projet `geonum_docker`. Si je voulais accéder à `app`, j’aurais deux choix :
...
**Merci à vous d’avoir tenu le coup, je sais que c’était dense.** Maintenant que vous avez une base solide pour votre carte, on va pouvoir ajouter des interactions avec celle-ci au cours des prochaines séances !
