...
Oui, c’est normal. Notre application a bien récupéré notre nœud "map" mais on ne lui a pas demandé d’afficher le résultat.
...
Cette méthode permet de dire à notre application d’**écouter** (*listen*) des **évènements** (*events*) et d’agit en conséquence en exécutant une **fonction**.

``` {.js code-line-numbers="|1,3|2|5"}
...
Et voilà ! Vous verrez dans le TP la méthode `.addLayer()` qui permet de faire l’inverse et rajouter une couche à la carte.
