...
-   Comprendre les évènements du navigateurs
...
[Cours : Ajouter des interactions à ma carte](./07_diapos_interactions.qmd){target="_blank"}

## TP – Gérer l’affichage de mes couches et ajouter une table attributaire
...
Pour référence, je vous mets ici le contenu de mon code au démarrage de ce TP. Vous noterez qu’il manque les éléments qu’on a ajoutés pendant le cours, alors repartez plutôt du votre !
...
![](https://media1.giphy.com/media/v1.Y2lkPTc5MGI3NjExODJjeW4zc3hidHd4aDUwOTBrcnFxNGVpdXJiM2RqdG1udTlwMXZ6eCZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/5pl3EHRYqMHwQ/giphy.gif){fig-alt="GIF : Cape d’invisibilité" fig-align="center"}
...
On souhaite maintenant interroger la couche deals en cliquant dessus. Cette étape est plus compliquée que la précédente car elle va nous demander d’utiliser `fetch()`. Normalement vous l’avez déjà abordé dans un autre cours, mais on va procéder petit à petit pour bien comprendre ce qu’on fait. Ouvrez l’exemple OpenLayers [WMS GetFeatureInfo (Image Layer)](https://openlayers.org/en/latest/examples/getfeatureinfo-image.html).
