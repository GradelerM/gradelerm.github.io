...
subtitle: "Cycle de développement, structure de l’application"
...
> Créez une carte grâce à **OpenLayers** à l’échelle globale. Vous pouvez utiliser le **fond de carte** que vous voulez SAUF celui par défaut (si vous êtes en manque d’inspiration, je vous conseille un des fonds de [Stamen Maps](https://maps.stamen.com/#watercolor/12/37.7706/-122.3782)). Ensuite, en partant du jeu de données [land_matrix_mines](http://localhost:4471/data/land_matrix/land_matrix_mines.gpkg) qu’on a utilisé la dernière fois, **affichez la couche de votre choix sur la carte**. Si vous avez besoin de la couche au format GeoJSON vous pouvez l’exporter comme telle depuis QGIS (pour rappel, on travaille en WGS84, il serait avisé d’exporter la couche dans ce système de projection). Enfin, si vous avez le temps, ajoutez **un titre** à votre carte.
...
Maintenant qu’on est prêts, on peut reprendre notre exemple et commencer à coder. On utilisera donc l’exemple **"[Single image WMS](https://openlayers.org/en/latest/examples/wms-image.html)"** d’OpenLayers qui nous permet d’ajouter une couche WMS depuis un GeoServer à notre application.
